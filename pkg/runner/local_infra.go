...
func healthcheck_common_local_infra(hcHelper HealthcheckHelper, ctx context.Context, ow *rpc.OutputWriter, cli *client.Client, controlNetworkID string, srcdir string, workdir string) {
	...
	hcHelper.Enlist("control-network",
		DockerNetworkChecker(ctx, ow, cli, controlNetworkID),
		DockerNetworkFixer(ctx, ow, cli, controlNetworkID),
	)

	// prometheus built from Dockerfile.
	hcHelper.Enlist("local-prometheus",
		DefaultContainerChecker(ctx, ow, cli, "testground-prometheus"),
		CustomContainerFixer(ctx, ow, cli, filepath.Join(srcdir, "infra/docker/testground-prometheus"), &ContainerFixerOpts{
	...
	hcHelper.Enlist("local-pushgateway",
		DefaultContainerChecker(ctx, ow, cli, "prometheus-pushgateway"),
		DefaultContainerFixer(ctx, ow, cli, &ContainerFixerOpts{
	...
	hcHelper.Enlist("local-grafana",
		DefaultContainerChecker(ctx, ow, cli, "testground-grafana"),
		DefaultContainerFixer(ctx, ow, cli, &ContainerFixerOpts{
	...
	hcHelper.Enlist("local-redis",
		DefaultContainerChecker(ctx, ow, cli, "testground-redis"),
		DefaultContainerFixer(ctx, ow, cli, &ContainerFixerOpts{
	...
	hcHelper.Enlist("local-redis-exporter",
		DefaultContainerChecker(ctx, ow, cli, "testground-redis-exporter"),
		DefaultContainerFixer(ctx, ow, cli, &ContainerFixerOpts{
